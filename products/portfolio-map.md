# Карта цифровых продуктов ridrid25 (инвентаризация 42 репо, 24.09.2026)

## Фундамент 1 — ФИНАНСЫ (основной, самый глубокий)
ГОТОВО / PRODUCTION:
- Test-claud-parametr: дашборд-витрина mp.ridfinance.ru; серверный
  мультиклиентский пайплайн WB/Ozon (API, ETL, SQLite, FastAPI);
  лендинг услуги «Экспресс-диагностика» (форма в бота, ниша m);
  «Советник» — AI-торговый агент (commerce-agents) с пилотом.
- data-smeta-audit — PRODUCTION (smeta.ridfinance.ru, реальный
  пользователь): выгодность тендера по смете ГРАНД-Смета. Тиражируем.
- taxwise-calculator — готовый интерактивный калькулятор
  УСН/АУСН/ОСНО с НДС, PDF-отчёт. Прямо в тему воронки.
- cfo-agent — бот-«финдиректор» по Google-таблице клиента, ~190
  тестов. Ядро идеи «цифры без посредника».
ОПУБЛИКОВАНО (адреса подтверждены 24.09):
- TaxWise: https://tax-wise-calculator-ru.lovable.app/
- CFO-дашборд: https://fin-dashboard-appscript.lovable.app/ —
  живая витрина для линии Б вне маркетплейсов (деньги, прибыль,
  ликвидность, ROE, ДЗ/КЗ). До созревания mirage — канонический.
- «Lovable Insights» — финмодель NPV/IRR/окупаемость (Lovable,
  published) — кандидат в демо оффера «финмодель под ключ».
- Контроль качества продаж: https://happysalescontrol.lovable.app/
  (happy-sales-trail: план-факт, маржа, оплаты, дебиторка, риск
  кассового разрыва) — вместе с Sales Motivation Hub и лендингом
  Motivation-offer образует оффер-кластер «продажи под контролем
  цифр» для собственника/РОП.
БЛИЗКО: mirage / «Mirage of profit» (прогнозный дашборд, не
опубликован), financial-command-center, happy-sales-trail («Remix of
Sales Motivation Hub» — published), vibefin-ai (концепт),
stage-analytics-suite.
КОНФЛИКТ С ПРАВИЛОМ «сервис, не персона» (11.09): cfo-outsourcing-
landing, rid-visit-site (+ частично Motivation-offer).
МЕРТВО: Ozon-wild-fintablo (пустой), DebtControl (дамп).

## Фундамент 2 — СТРОЙКА/ПРОИЗВОДСТВО B2B (недооценённый high-ticket)
- data-smeta-audit (production, см. выше);
- asphalt-profit-master (актуальная итерация) + uds-asphalt-calc
  (прошлая; asphalt-margin-calculator — пустой);
- Parametric-transport-budget — готовая финмодель транспортного
  бюджета (Sheets + HTML-дашборд);
- SprayCheck (= spraycheck-vision-pro, дубль) — агро-прототип.
ЦА: подрядчики, АБЗ, транспортные компании. Естественный чек
50–300 тыс. за внедрение/настройку.

## Фундамент 3 — ИГРЫ
Chroma Loop (dream/adventures) — активная; island-hop, jumpbot,
cyber-blue — витрины/игрушки.

## Инфраструктура и личное (не продукты)
- ai-planner-balance опубликован (https://ridrid25.github.io/
  ai-planner-balance/), но остаётся личным инструментом: интерфейс
  захардкожен под владелицу («Доброе утро, Татьяна») — в воронку и
  витрины не включать; публичная ссылка с именем расходится с
  каноном «сервис, не персона».
rid-finance (контент-система: автопостинг, бот заявок), vibepost /
content-factory-personal, telegram-bots (реестр 20 ботов),
pomoshchnik-naznacheniya-vstrech («Calendly в Telegram», продвинут),
ai-planner-balance (личный).

## Фундамент 1а — СКИЛЛЫ ЭКСПЕРТА (новая категория, 24.09.2026)
Собственные скиллы Claude Code/Codex (установлены локально у заказчицы):
- financial-model-builder — 5 режимов: интейк-опросник клиента,
  проектирование архитектуры финмодели, сборка, аудит чужой модели,
  презентация для собственника/инвестора. Роль двойная:
  (а) производственный инструмент линии Б — «финмодель под ключ»
  и «аудит модели» делаются в разы быстрее, себестоимость услуги
  с чеком 30-100 тыс. падает; (б) самостоятельный товар для ТРЕТЬЕЙ
  аудитории — финансистов/экспертов, осваивающих ИИ (растущая ниша,
  чек 3-10 тыс. за скилл с инструкцией).
- integration-builder — 6 шагов сборки интеграции между сервисами
  (бриф, разведка API, ключи, план, код, проверка). Внутренний
  ускоритель: интеграции пайплайна, ботов, клиентских внедрений.
Статус: не в облачной синхронизации — чтобы применять их в этих
сессиях и версионировать, положить в репозиторий (.claude/skills/)
или прислать ZIP.

## VIBEFIN — зонтичный оффер линии Б (формула заказчицы, 24.09.2026)
«Связь 1С / 1С:Фреш / Google Sheets — в понятный дашборд».
Сборка из готовых частей:
- витрина идеи: vibefin-ai (концепт-сайт: утренняя сводка, риски,
  сценарии);
- витрина результата: https://fin-dashboard-appscript.lovable.app/
  (как выглядят «понятные цифры»);
- производство подключений: скилл integration-builder (бриф →
  разведка API → ключи → план → код → проверка) — 1С/Фреш имеют
  REST/OData, Google Sheets — API; каждое внедрение = прогон скилла;
- модели и презентации: скилл financial-model-builder;
- квалификация клиента: анкета (3 вопроса) → переписка.
Монетизация: внедрение 30–100 тыс. (разово) + сопровождение
(подписка, по аналогии с 3 900 руб./мес у селлеров, для 1С-сегмента
выше). Аудитория: малый/средний бизнес с 1С и таблицами, без
финдиректора.

## Решения
1. В воронку линии А добавить taxwise-calculator (апселл/вторая
   ступень к налоговому гайду, письмо 4–5).
2. Линия Б строится на готовом: экспресс-диагностика (лендинг есть,
   перевести с персоны на сервис) → сопровождение на серверном
   пайплайне → cfo-agent/Советник как подписка.
3. Стройка-B2B — отдельная линия высокого чека: тираж data-smeta-
   audit + пакет АБЗ/транспорт. Не смешивать с селлерской.
4. Гигиена: выбрать канонический CFO-дашборд (кандидат mirage или
   financial-command-center), заархивировать дубли (spraycheck-
   vision-pro, старые asphalt, пустые), лендинги персоны — переделать
   или закрыть.
5. Скиллы: financial-model-builder закладывается в оффер линии Б
   «финмодель под ключ» как ускоритель производства; отдельно —
   кандидат в товар для аудитории экспертов-вайбкодеров.
