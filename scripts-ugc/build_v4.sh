#!/bin/bash
# Chroma Loop UGC reel v4 — max-energy edit: cleaned game audio (LF rumble removed),
# 2.5x speed ramps, reaction micro-cuts from Veo clips with invert-pop, speed lines,
# confetti burst, camera jitter/rotation, glitch transitions, freeze-frame CTA.
# Timeline (s): 0-2 hook | 2-6.4 lvl3 2.5x | 6.4-6.93 reaction1 | 6.93-11.33 lvl4 2.5x
#   11.33-13.33 slow-mo | 13.33-13.93 flash2 | 13.93-14.47 reaction2 | 14.47-17.47 ocean
#   17.47-20 win freeze. 600 frames total.
set -euo pipefail
cd "$(dirname "$0")"

SAT="eq=saturation=1.25:contrast=1.03"
RSAT="eq=brightness=0.06:saturation=1.35:contrast=1.05"
ACLEAN="highpass=f=220:poles=2,highpass=f=220:poles=2,afftdn=nr=18:nf=-38,agate=threshold=0.015:ratio=3:attack=2:release=150,lowpass=f=14000,volume=6dB"
...
# A: rapid hook — three flashes + white frames, cleaned audio
...
[0:a]atrim=10.2:10.8,asetpts=PTS-STARTPTS,${ACLEAN},aresample=48000,aformat=channel_layouts=stereo[q1];\
[1:a]atrim=11.5:12.1,asetpts=PTS-STARTPTS,${ACLEAN},aresample=48000,aformat=channel_layouts=stereo[q2];\
[0:a]atrim=30.1:30.7667,asetpts=PTS-STARTPTS,${ACLEAN},aresample=48000,aformat=channel_layouts=stereo[q3];\
...
[p1][q1][w1][s1][p2][q2][w2][s2][p3][q3]concat=n=5:v=1:a=1[v][a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 2.0 v4segA.mp4
...
[0:a]atrim=0.2:11.2,asetpts=PTS-STARTPTS,${ACLEAN},atempo=2.5,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 4.4 v4segB.mp4

# R1: reaction micro-cut (clip2, smile) — pull-back zoom, invert pop on entry
ffmpeg -y -v error -i "$IN/clip2.mp4" -filter_complex "\
[0:v]trim=5.8:6.3333,setpts=PTS-STARTPTS,fps=30,scale=1080:1920,${RSAT},\
scale=w='trunc(1080*(1.30-0.28*t)/2)*2':h='trunc(1920*(1.30-0.28*t)/2)*2':eval=frame,\
crop=1080:1920:x='(iw-ow)/2':y='(ih-oh)/2',negate=enable='lt(t,0.067)',setsar=1,format=yuv420p[v];\
anullsrc=r=48000:cl=stereo:d=0.5333[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 0.5333 v4segR1.mp4

# C: lvl4 at 2.5x, zoom drift + gentle camera sway
ffmpeg -y -v error -i gp_forest.mp4 -filter_complex "\
...
[0:a]atrim=18.4:29.4,asetpts=PTS-STARTPTS,${ACLEAN},atempo=2.5,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 4.4 v4segC.mp4

# D: slow-mo 0.4x from the raw 120 fps recording, silent (riser is added in stage 2)
...
crop=1080:1920:x='(iw-ow)/2':y='(ih-oh)/2',setsar=1[v];\
anullsrc=r=48000:cl=stereo:d=2.0[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 2.0 v4segD.mp4

# E: forest flash2 at 1x — big punch + shake + rotation jitter
ffmpeg -y -v error -i gp_forest.mp4 -filter_complex "\
[0:v]trim=30.4:31.0,setpts=PTS-STARTPTS,${SAT},\
...
[0:a]atrim=30.4:31.0,asetpts=PTS-STARTPTS,${ACLEAN},aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 0.6 v4segE.mp4

# R2: reaction micro-cut (clip1, surprised) — invert pop
ffmpeg -y -v error -i "$IN/clip1.mp4" -filter_complex "\
[0:v]trim=0.8:1.3333,setpts=PTS-STARTPTS,fps=30,scale=1080:1920,${RSAT},\
scale=w='trunc(1080*(1.28-0.23*t)/2)*2':h='trunc(1920*(1.28-0.23*t)/2)*2':eval=frame,\
crop=1080:1920:x='(iw-ow)/2':y='(ih-oh)/2',negate=enable='lt(t,0.067)',setsar=1,format=yuv420p[v];\
anullsrc=r=48000:cl=stereo:d=0.5333[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 0.5333 v4segR2.mp4
...
[0:a]atrim=10.4:13.4,asetpts=PTS-STARTPTS,${ACLEAN},aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 3.0 v4segF.mp4
...
[0:v]fps=30,${SAT},\
scale=w='trunc(1080*(1.04+0.10*t/2.53+0.02*sin(2*PI*1.4*t))/2)*2':h='trunc(1920*(1.04+0.10*t/2.53+0.02*sin(2*PI*1.4*t))/2)*2':eval=frame,\
crop=1080:1920:x='(iw-ow)/2':y='(ih-oh)/2',setsar=1[v]" \
 -map "[v]" -map 1:a $VOUT $AOUT -t 2.5333 v4segG.mp4

# --- Stage 2: concat + overlays (speed lines, confetti, captions) + SFX ------
ffmpeg -y -v error \
 -i v4segA.mp4 -i v4segB.mp4 -i v4segR1.mp4 -i v4segC.mp4 -i v4segD.mp4 \
 -i v4segE.mp4 -i v4segR2.mp4 -i v4segF.mp4 -i v4segG.mp4 \
 ...
 -filter_complex "\
[0:v][0:a][1:v][1:a][2:v][2:a][3:v][3:a][4:v][4:a][5:v][5:a][6:v][6:a][7:v][7:a][8:v][8:a]concat=n=9:v=1:a=1[vc][ac];\
[vc]rgbashift=rh=10:bh=-10:gv=6:enable='between(t,6.37,6.47)+between(t,11.28,11.40)+between(t,14.42,14.52)+between(t,17.42,17.52)'[vg];\
[9:v]format=rgba,colorchannelmixer=aa=0.85[sl];\
[vg][sl]overlay=x=0:y=0:enable='between(t,0.25,0.42)+between(t,0.90,1.07)+between(t,1.60,1.77)+between(t,6.10,6.32)+between(t,13.42,13.68)+between(t,15.74,15.98)'[ov0];\
[10:v]format=rgba,setpts=PTS+15.77/TB[cf];\
[ov0][cf]overlay=x=0:y=0:enable='between(t,15.77,18.27)'[ov1];\
[11:v]format=rgba,fade=in:st=0.10:d=0.12:alpha=1,fade=out:st=1.85:d=0.10:alpha=1[c1];\
[12:v]format=rgba,fade=in:st=2.30:d=0.15:alpha=1,fade=out:st=6.05:d=0.15:alpha=1[c2];\
[13:v]format=rgba,fade=in:st=7.10:d=0.15:alpha=1,fade=out:st=11.05:d=0.15:alpha=1[c3];\
[14:v]format=rgba,fade=in:st=11.45:d=0.15:alpha=1,fade=out:st=13.15:d=0.15:alpha=1[c4];\
[15:v]format=rgba,fade=in:st=15.80:d=0.10:alpha=1,fade=out:st=17.05:d=0.15:alpha=1[c5];\
[16:v]format=rgba,fade=in:st=17.60:d=0.15:alpha=1[c6];\
[17:v]format=rgba,fade=in:st=17.75:d=0.15:alpha=1[c7];\
[ov1][c1]overlay=x=0:y='430+50*exp(-14*(t-0.10))':enable='between(t,0.10,1.95)'[o1];\
[o1][c2]overlay=x=0:y='430+50*exp(-14*(t-2.30))':enable='between(t,2.30,6.20)'[o2];\
...
sine=frequency=650:duration=0.12:sample_rate=48000,afade=t=out:st=0.01:d=0.11,volume=0.5,aformat=channel_layouts=stereo,adelay=6400|6400[pop1];\
...
sine=frequency=650:duration=0.12:sample_rate=48000,afade=t=out:st=0.01:d=0.11,volume=0.5,aformat=channel_layouts=stereo,adelay=13930|13930[pop2];\
...
[ac][d1][d2][d3][w1][w2][d4][pop1][w3][riser][big1][pop2][w4][big2][pp1][pp2][d5]amix=inputs=17:duration=first:normalize=0[am]" \
 -map "[vt]" -map "[am]" $VOUT $AOUT v4_pre.mp4

# --- Loudness: linear loudnorm to PCM, exact trim, encode once ---------------
ffmpeg -y -i v4_pre.mp4 -af loudnorm=I=-14:TP=-2:LRA=11:print_format=json -f null - 2>&1 \
  | sed -n '/^{/,/^}/p' > loudnorm4.json
II=$(grep '"input_i"' loudnorm4.json | sed 's/[^0-9.-]*//g')
TP=$(grep '"input_tp"' loudnorm4.json | sed 's/[^0-9.-]*//g')
LRA=$(grep '"input_lra"' loudnorm4.json | sed 's/[^0-9.-]*//g')
TH=$(grep '"input_thresh"' loudnorm4.json | sed 's/[^0-9.-]*//g')
OFF=$(grep '"target_offset"' loudnorm4.json | sed 's/[^0-9.-]*//g')
ffmpeg -y -v error -i v4_pre.mp4 \
  ...
  -c:v copy -c:a pcm_s16le -f matroska v4_ln.mkv
D=$(ffmpeg -i v4_ln.mkv -af loudnorm=print_format=json -f null - 2>&1 | grep '"input_i"' | sed 's/[^0-9.-]*//g')
CORR=$(python3 -c "print(round(-14 - ($D), 2))")
echo "post-loudnorm I=$D corr=${CORR}dB"
ffmpeg -y -v error -i v4_ln.mkv \
  ...
  chroma_ugc_reels_v4.mp4

ffprobe -v error -show_entries format=duration -show_entries stream=codec_name,profile,width,height,r_frame_rate -of default=noprint_wrappers=1 chroma_ugc_reels_v4.mp4
ffmpeg -i chroma_ugc_reels_v4.mp4 -af loudnorm=print_format=json -f null - 2>&1 | grep -E '"input_i"|"input_tp"'
