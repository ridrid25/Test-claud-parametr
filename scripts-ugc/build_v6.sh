...
[p1][q1][w1][s1][p2][q2][w2][s2][p3][q3]concat=n=5:v=1:a=1[v][a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 2.0 v6segA.mp4
...
[0:a]atrim=0.2:11.2,asetpts=PTS-STARTPTS,atempo=2.5,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 4.4 v6segB.mp4
...
[0:a]atrim=10.75:11.2833,asetpts=PTS-STARTPTS,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 0.5333 v6segB2.mp4
...
[0:a]atrim=18.4:29.4,asetpts=PTS-STARTPTS,atempo=2.5,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 4.4 v6segC.mp4
...
[0:a]asetpts=PTS-STARTPTS,atempo=0.5,atempo=0.8,volume=0.6,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 2.0 v6segD.mp4
...
[0:a]atrim=30.4:31.5333,asetpts=PTS-STARTPTS,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 1.1333 v6segE.mp4
...
[0:a]atrim=10.4:13.4,asetpts=PTS-STARTPTS,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 3.0 v6segF.mp4
...
crop=1080:1920:x='(iw-ow)/2':y='(ih-oh)/2',setsar=1[v]" \
 -map "[v]" -map 1:a $VOUT $AOUT -t 2.5333 v6segG.mp4

# --- Stage 2: concat + overlays + SFX ----------------------------------------
ffmpeg -y -v error \
 -i v6segA.mp4 -i v6segB.mp4 -i v6segB2.mp4 -i v6segC.mp4 -i v6segD.mp4 \
 -i v6segE.mp4 -i v6segF.mp4 -i v6segG.mp4 \
...
[ac][d1][d2][d3][w1][w2][d4][w3][riser][big1][w4][big2][pp1][pp2][d5]amix=inputs=15:duration=first:normalize=0[am]" \
 -map "[vt]" -map "[am]" $VOUT $AOUT v6_pre.mp4

# --- Loudness -----------------------------------------------------------------
ffmpeg -y -i v6_pre.mp4 -af loudnorm=I=-14:TP=-2:LRA=11:print_format=json -f null - 2>&1 \
  | sed -n '/^{/,/^}/p' > loudnorm6.json
II=$(grep '"input_i"' loudnorm6.json | sed 's/[^0-9.-]*//g')
TP=$(grep '"input_tp"' loudnorm6.json | sed 's/[^0-9.-]*//g')
LRA=$(grep '"input_lra"' loudnorm6.json | sed 's/[^0-9.-]*//g')
TH=$(grep '"input_thresh"' loudnorm6.json | sed 's/[^0-9.-]*//g')
OFF=$(grep '"target_offset"' loudnorm6.json | sed 's/[^0-9.-]*//g')
ffmpeg -y -v error -i v6_pre.mp4 \
  ...
  -c:v copy -c:a pcm_s16le -f matroska v6_ln.mkv
D=$(ffmpeg -i v6_ln.mkv -af loudnorm=print_format=json -f null - 2>&1 | grep '"input_i"' | sed 's/[^0-9.-]*//g')
CORR=$(python3 -c "print(round(-14 - ($D), 2))")
echo "post-loudnorm I=$D corr=${CORR}dB"
ffmpeg -y -v error -i v6_ln.mkv \
  ...
  chroma_ugc_reels_v6.mp4

ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1 chroma_ugc_reels_v6.mp4
ffmpeg -i chroma_ugc_reels_v6.mp4 -af loudnorm=print_format=json -f null - 2>&1 | grep -E '"input_i"|"input_tp"'
