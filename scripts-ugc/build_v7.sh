...
# A: rapid hook — three flashes + white frames
...
[0:v]trim=30.1:30.7667,setpts=PTS-STARTPTS,${SAT},crop='trunc(iw/1.08/2)*2':'trunc(ih/1.08/2)*2',scale=540:960,setsar=1,split=4[k1][k2][k3][k4];\
[k2]hflip[k2f];[k3]vflip[k3f];[k4]hflip,vflip[k4f];\
color=black:s=1080x1920:d=0.6667:r=30,format=yuv420p[kbg];\
...
[p1][q1][w1][s1][p2][q2][w2][s2][p3][q3]concat=n=5:v=1:a=1[v][a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 2.0 v7segA.mp4
...
[0:a]atrim=0.2:11.2,asetpts=PTS-STARTPTS,atempo=2.5,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 4.4 v7segB.mp4
...
[0:a]atrim=10.75:11.2833,asetpts=PTS-STARTPTS,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 0.5333 v7segB2.mp4
...
[0:a]atrim=18.4:29.4,asetpts=PTS-STARTPTS,atempo=2.5,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 4.4 v7segC.mp4
...
[0:a]asetpts=PTS-STARTPTS,atempo=0.5,atempo=0.8,volume=0.6,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 2.0 v7segD.mp4
...
[0:a]atrim=30.4:31.5333,asetpts=PTS-STARTPTS,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 1.1333 v7segE.mp4
...
[0:a]atrim=10.4:13.4,asetpts=PTS-STARTPTS,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 3.0 v7segF.mp4
...
crop=1080:1920:x='(iw-ow)/2':y='(ih-oh)/2',setsar=1[v]" \
 -map "[v]" -map 1:a $VOUT $AOUT -t 2.5333 v7segG.mp4

# --- Stage 2: concat + overlays + SFX ----------------------------------------
ffmpeg -y -v error \
 -i v7segA.mp4 -i v7segB.mp4 -i v7segB2.mp4 -i v7segC.mp4 -i v7segD.mp4 \
 -i v7segE.mp4 -i v7segF.mp4 -i v7segG.mp4 \
...
[ve3]negate=enable='between(t,13.45,13.52)+between(t,15.77,15.84)'[vg];\
...
[10:v]format=rgba,fade=in:st=0.10:d=0.12:alpha=1,fade=out:st=1.85:d=0.10:alpha=1[c1];\
[11:v]format=rgba,fade=in:st=2.30:d=0.15:alpha=1,fade=out:st=6.05:d=0.15:alpha=1[c2];\
...
[ac][d1][d2][d3][w1][w2][d4][w3][riser][big1][w4][big2][pp1][pp2][d5]amix=inputs=15:duration=first:normalize=0[am]" \
 -map "[vt]" -map "[am]" $VOUT $AOUT v7_pre.mp4

# --- Loudness -----------------------------------------------------------------
ffmpeg -y -i v7_pre.mp4 -af loudnorm=I=-14:TP=-2:LRA=11:print_format=json -f null - 2>&1 \
  | sed -n '/^{/,/^}/p' > loudnorm7.json
II=$(grep '"input_i"' loudnorm7.json | sed 's/[^0-9.-]*//g')
TP=$(grep '"input_tp"' loudnorm7.json | sed 's/[^0-9.-]*//g')
LRA=$(grep '"input_lra"' loudnorm7.json | sed 's/[^0-9.-]*//g')
TH=$(grep '"input_thresh"' loudnorm7.json | sed 's/[^0-9.-]*//g')
OFF=$(grep '"target_offset"' loudnorm7.json | sed 's/[^0-9.-]*//g')
ffmpeg -y -v error -i v7_pre.mp4 \
  ...
  -c:v copy -c:a pcm_s16le -f matroska v7_ln.mkv
D=$(ffmpeg -i v7_ln.mkv -af loudnorm=print_format=json -f null - 2>&1 | grep '"input_i"' | sed 's/[^0-9.-]*//g')
CORR=$(python3 -c "print(round(-14 - ($D), 2))")
echo "post-loudnorm I=$D corr=${CORR}dB"
ffmpeg -y -v error -i v7_ln.mkv \
  ...
  chroma_ugc_reels_v7.mp4

ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1 chroma_ugc_reels_v7.mp4
ffmpeg -i chroma_ugc_reels_v7.mp4 -af loudnorm=print_format=json -f null - 2>&1 | grep -E '"input_i"|"input_tp"'
