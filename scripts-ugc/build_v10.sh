...
[h][ha][p1][q1][w1][s1][p2][q2][w2][s2][p3][q3]concat=n=6:v=1:a=1[v][a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 2.0 v10segA.mp4
...
[0:a]atrim=0.2:11.2,asetpts=PTS-STARTPTS,atempo=2.5,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 4.4 v10segB.mp4
...
[0:a]atrim=10.75:11.2833,asetpts=PTS-STARTPTS,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 0.5333 v10segB2.mp4
...
[0:a]atrim=18.4:29.4,asetpts=PTS-STARTPTS,atempo=2.5,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 4.4 v10segC.mp4
...
[0:a]asetpts=PTS-STARTPTS,atempo=0.5,atempo=0.8,volume=0.6,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 2.0 v10segD.mp4
...
[0:a]atrim=30.4:31.5333,asetpts=PTS-STARTPTS,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 1.1333 v10segE.mp4
...
[0:a]atrim=10.4:13.4,asetpts=PTS-STARTPTS,aresample=48000,aformat=channel_layouts=stereo[a]" \
 -map "[v]" -map "[a]" $VOUT $AOUT -t 3.0 v10segF.mp4
...
crop=1080:1920:x='(iw-ow)/2':y='(ih-oh)/2',setsar=1[v]" \
 -map "[v]" -map 1:a $VOUT $AOUT -t 2.5333 v10segG.mp4

# --- Stage 2: concat + overlays + SFX ----------------------------------------
ffmpeg -y -v error \
 -i v10segA.mp4 -i v10segB.mp4 -i v10segB2.mp4 -i v10segC.mp4 -i v10segD.mp4 \
 -i v10segE.mp4 -i v10segF.mp4 -i v10segG.mp4 \
 ...
 -i /home/user/Test-claud-parametr/input/clip1.mp4 -i /home/user/Test-claud-parametr/input/clip2.mp4 -i /home/user/Test-claud-parametr/input/clip3.mp4 \
 -filter_complex "\
...
[18:a]atrim=0:2.3,asetpts=PTS-STARTPTS,afade=t=out:st=2.0:d=0.3,aresample=48000,aformat=channel_layouts=stereo[vo1];\
[19:a]atrim=0:4.2,asetpts=PTS-STARTPTS,afade=t=out:st=3.9:d=0.3,aresample=48000,aformat=channel_layouts=stereo,adelay=7350|7350[vo2];\
[20:a]atrim=0:4.8,asetpts=PTS-STARTPTS,afade=t=out:st=4.5:d=0.3,aresample=48000,aformat=channel_layouts=stereo,adelay=15000|15000[vo3];\
[ac][buzz][d1][d2][d3][w1][w2][d4][w3][riser][big1][w4][big2][pp1][pp2][d5][d6][d7][vo1][vo2][vo3]amix=inputs=21:duration=first:normalize=0[am]" \
 -map "[vt]" -map "[am]" $VOUT $AOUT v10_pre.mp4

# --- Loudness -----------------------------------------------------------------
ffmpeg -y -i v10_pre.mp4 -af loudnorm=I=-14:TP=-2:LRA=11:print_format=json -f null - 2>&1 \
  | sed -n '/^{/,/^}/p' > loudnorm10.json
II=$(grep '"input_i"' loudnorm10.json | sed 's/[^0-9.-]*//g')
TP=$(grep '"input_tp"' loudnorm10.json | sed 's/[^0-9.-]*//g')
LRA=$(grep '"input_lra"' loudnorm10.json | sed 's/[^0-9.-]*//g')
TH=$(grep '"input_thresh"' loudnorm10.json | sed 's/[^0-9.-]*//g')
OFF=$(grep '"target_offset"' loudnorm10.json | sed 's/[^0-9.-]*//g')
ffmpeg -y -v error -i v10_pre.mp4 \
  ...
  -c:v copy -c:a pcm_s16le -f matroska v10_ln.mkv
D=$(ffmpeg -i v10_ln.mkv -af loudnorm=print_format=json -f null - 2>&1 | grep '"input_i"' | sed 's/[^0-9.-]*//g')
CORR=$(python3 -c "print(round(-14 - ($D), 2))")
echo "post-loudnorm I=$D corr=${CORR}dB"
ffmpeg -y -v error -i v10_ln.mkv \
  ...
  chroma_ugc_reels_v10.mp4

ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1 chroma_ugc_reels_v10.mp4
ffmpeg -i chroma_ugc_reels_v10.mp4 -af loudnorm=print_format=json -f null - 2>&1 | grep -E '"input_i"|"input_tp"'
